...
export const getCacheProps = async ({
  locale,
  cacheBucketName,
  cacheFolder,
}: {
...
export const getCopyTunerCacheProps = functions().https.onCall(
  async ({ locale }: { locale: string }) => {
    return await getCacheProps({ locale, cacheBucketName, cacheFolder });
  }
);
